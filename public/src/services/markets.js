...
export const Markets = (() => {
  ...
  async function _fetchPreferredRound() {
    ...
    const cached = AppCache?.get("round:preferred");
    ...
    const { data: openRound, error: openErr } = await sb
      .from("bet_rounds")
      .select("*, markets(*)")
      .eq("status", "open")
      .gt("ends_at", nowIso)
    ...
    if (!openErr && openRound) {
      AppCache?.set("round:preferred", { v: openRound }, 30_000);
      return openRound;
    }

    // 2) Then a recently locked round (shows "resolving" while settlement runs).
    const { data: lockedRound, error: lockedErr } = await sb
      .from("bet_rounds")
      .select("*, markets(*)")
      .eq("status", "locked")
    ...
    if (!lockedErr && lockedRound) {
      AppCache?.set("round:preferred", { v: lockedRound }, 30_000);
      return lockedRound;
    }

    // 3) Finally, next upcoming round.
    const { data: upcomingRound, error: upcomingErr } = await sb
      .from("bet_rounds")
      .select("*, markets(*)")
      .eq("status", "upcoming")
    ...
    if (!upcomingErr && upcomingRound) {
      AppCache?.set("round:preferred", { v: upcomingRound }, 30_000);
      return upcomingRound;
    }

    // No round found — cache null too so rapid retries don't hammer Supabase
    AppCache?.set("round:preferred", { v: null }, 15_000);
    return null;
  }
  ...
  function _enterRound() {
    const container = document.getElementById("markets-container");
    if (container) container.classList.remove("mkts-round-hidden");
    window.dispatchEvent(new CustomEvent('banners:hide'));
  }
