...
async function saveProfile() {
  ...
  try {
    ...
    const { error: upsertError } = await sb.from("profiles").upsert(payload, { onConflict: "user_id" });
    if (upsertError) throw upsertError;

    const { error: authError } = await sb.auth.updateUser({ data: { username, avatar_url } });
