...
function onAnimationFrame(now) {
  ...
  if (vt !== _lastVideoTime) { _lastVideoTime = vt; addSample(now); render(); }
  _rafId = requestAnimationFrame(onAnimationFrame);
}
...
function computeFps() {
  ...
  return (!Number.isFinite(avgMs) || avgMs <= 0) ? null : 1000 / avgMs;
}
...
function destroy() {
  _running = false; _samples = []; _lastNow = 0; _lastVideoTime = -1;
...
function reset() {
  _samples = []; _lastNow = 0; _lastVideoTime = -1;
  if (_el) { _el.textContent = 'FPS --.-'; _el.className = _el.className.replace(/fps-\w+/g, '').trim(); }
}

export const FpsOverlay = { init, destroy, reset };
