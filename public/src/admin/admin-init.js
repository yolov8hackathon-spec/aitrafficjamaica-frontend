...
import { AdminMapping } from './admin-mapping.js';
import { AdminModel } from './admin-model.js';
import { AdminStreams } from './admin-streams.js';
import { AdminZones } from './admin-zones.js';
import { WidgetLayout } from './admin-widget-layout.js';
  ...
  try {
    ...
    if (roundEl) {
      if (round?.status) {
        ...
        const { data: fallbackRound } = await sb
          .from("bet_rounds")
          .select("status")
...
function initDetSubnav() {
  ...
  function activateTab(target) {
    ...
    if (target === "zones") {
      ...
      setTimeout(() => AdminLine?.refresh?.(), 0);
      setTimeout(() => AdminLine?.refresh?.(), 180);
    }
  }
...
async function loadMlProgress() {
  ...
  try {
    ...
    const [{ count: totalRows }, { count: rows24h }, recentResp, activeModelResp, lastJobsResp] = await Promise.all([
      sb.from("ml_detection_events").select("id", { count: "exact", head: true }),
      sb.from("ml_detection_events").select("id", { count: "exact", head: true }).gte("captured_at", since24h),
      sb
        .from("ml_detection_events")
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("admin:panel-change", (e) => {
    ...
    if (panel === "banners") { AdminBanners?.init(); AdminBanners?.load(); }
    if (panel === "cameras") { AdminStreams?.init(); }
    if (panel === "detection") {
...
(function mlPipelineSyncInit() {
  ...
  function syncMlPipelineCards() {
    ...
    const captureState = (mlCaptureStats || mlCaptureStats || {});
...
(function mlPipelineStageHealthInit() {
  ...
  function syncStageHealth() {
    const cap = (mlCaptureStats || mlCaptureStats || {});
